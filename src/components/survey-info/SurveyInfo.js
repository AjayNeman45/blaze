...
function SurveyInfo() {
			...
			.then(data => {
				...
				if (data?.status?.toLowerCase() === "bidding") {
					...
					if (data?.internal_status.toLowerCase() === "soft launch")
						...
						data?.internal_status.toLowerCase() === "full launch"
					)
						setBatti(4)
	...
	const handleChangeSurveyNameBtn = e => {
		...
		let changes = survey?.changes ? survey?.changes : []
		changes.push({
			changed_by: {
			...
			<div className={styles.survey_info_container}>
				<div className={styles.survey_name_and_btns}>
					<div className={styles.survey_info_name}>
						<h1
							onClick={() =>
								setSurveyNameEditModal(!surveyNameEditModal)
							}
						>
							{newSurveyName}
							<span
              ...
              {statusOptions?.map((option) => (
                <option value={option.value}>{option.label}</option>
						...
						<button
							...
							onClick={() =>
								history.push(`/surveys/reports/${surveyID}`)
							}
						>
							Get Reports
						</button>
              ...
              <GoPrimitiveDot /> {survey?.status}
					...
					<div className={styles.status}>
						<ul>
							...
							<li
								className={
									batti >= 6 ? styles.active : style.inactive
								}
							>
								<GoPrimitiveDot />
								closed & billed{" "}
			...
			{surveyCloneModal && (
				...
				>
					<div className={styles.clonedMsgModalStyle}>
						...
						<div className={styles.btns}>
							<button onClick={() => setSurveyCloneModal(false)}>
								Close
							</button>
							<p
