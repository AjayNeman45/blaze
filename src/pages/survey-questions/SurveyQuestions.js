...
const SurveyQuestions = () => {
		...
		if (flag) {
			...
			if (question?.conditions?.quotas?.hasOwnProperty(index)) {
				if (!(question?.conditions?.quotas?.[index] - cntCompleted)) {
					...
					setErrCode(40)
					setErrMsg(
						"Respondent went over quota within the Lucid Marketplac"
					)
	...
	const handleMultiPunch = () => {
		...
		if (multiPunchResp.length < question?.conditions?.how_many?.min) {
			...
			multiPunchResp.length > question?.conditions?.how_many?.max
		) {
			if (gamma === "alpha") history.push(predirectUrl)
			else
				setError(
	...
	const handleNumericOpenEnd = () => {
		...
		if (flag) {
			...
			isFinalQuestion ? (mirats_status = 3) : (mirats_status = 1)
			if (
				question?.conditions?.quotas?.hasOwnProperty(optIndexForUserAns)
			) {
				if (
					!(
						question?.conditions?.quotas[optIndexForUserAns] -
						cntCompleted
					)
				) {
					mirats_status = 40
	...
	useEffect(() => {
		...
		const func = async () => {
			...
			if (session.data()?.mirats_status === 40) {
				setErrCode(40)
				setErrMsg(
					"Respondent went over quota within the Lucid Marketplace"
				)
				return
			}
			...
			getSurvey(surveyID)
				.then(async data => {
					moveToQuestionNo =
						data?.qualifications?.questions[0]?.question_id
						...
						if (question_id !== responseOfQId) {
							...
							setQuestion(oldData => {
								return {
									...
									question_type:
										ql_question.data()?.question_type,
									is_core_demographic:
										ql_question.data()?.is_core_demographic,
									question_name: ql_question.data()?.name,
								}
							})
							if (
								questions[i + 1]?.status &&
								questions[i + 1]?.question_id
							) {
			...
			{!loading && !errMsg && !errCode && (
				<div className='qualification_question_page'>
					<img
						src={Logo}
						alt=''
						className='qualification-page-logo'
					/>
