...
const EndPoint = () => {
	...
	const decodedID = hashids.decode(
		id.includes("Test") ? id.split("-")[1] : id
	)
	useEffect(() => {
				...
				sessions.forEach(session => {
					...
					if (
						...
						updateSession(decodedID[0], session.id, gamma, body)
							.then(() => {
								console.log(
									"session updated with client response"
								)
							})
			...
			<div className={styles.survey_complete_page}>
				<div className={styles.thank_you_container}>
					...
					<p className={styles.desc}>
						Thank you very much for your participation. Your points
						will credit in your account. We appreciate you inputs.
						In case of any issue you've faced during the survey or
						you want to submit feedback to our team, please email us
						at
					</p>
					<a
						href='https://mail.google.com/mail/?view=cm&fs=1&to=support_globalsurvey@miratsinsights.com'
				...
				<div
					...
					onClick={() =>
						window.open("https://miratsinsights.com", "_blank")
					}
				>
					<p>
						{/* <a href='https://miratsinsights.com' target='_blank'> */}
	...
	switch (String(status)) {
		...
		case "30":
			if (redirects?.hasOwnProperty("quality_terminate")) {
				...
				if (redirects?.quality_terminate.includes("surveyNumber")) {
					x =
						redirects?.quality_terminate.split(
							"[%surveyNumber%]"
						)[0] + surveyID
				}
				y = x.split("[%rid%]")[0] + rid + x.split("[%rid%]")[1]
				...
				if (redirects?.terminate.includes("surveyNumber")) {
					x =
						redirects?.terminate.split("[%surveyNumber%]")[0] +
						surveyID
				}
				y = x.split("[%rid%]")[0] + rid + x.split("[%rid%]")[1]
			}
		...
		case "40":
			...
			if (redirects?.quota_full.includes("surveyNumber")) {
				x =
					redirects?.quota_full.split("[%surveyNumber%]")[0] +
					surveyID
			}
			y = x.split("[%rid%]")[0] + rid + x.split("[%rid%]")[1]
			break
