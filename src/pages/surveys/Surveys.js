...
const Surveys = () => {
	...
	const filterSurveys = () => {
		Object.keys(filters).forEach(key => {
			if (key === "study_type" || key === "survey_type") {
				...
				setCurrentSurveys(prevData => {
					return prevData.filter(survey => {
						if (survey?.country?.country === filters[key]?.value)
							return survey
					})
				})
			} else if (key === "client") {
	...
	const appliedFilterDelete = key => {
		setFilters(prevData => {
			let newData = {}
			Object.keys(prevData)?.map(fkey => {
				if (fkey !== key) {
					newData[fkey] = prevData[fkey]
				}
			})
			return newData
		})
						...
						<div className={styles.left_header}>
							<div className={styles.head}>
								...
								<Switch
									onChange={e => {
										if (e.target.checked)
											history.push("/mi/projects")
										else
											history.push("/mi/surveys?view=all")
									}}
					...
					<div className={styles.right}>
						<div className={styles.id_card}>
							...
							<div className={styles.id_card_description}>
								<p>
									Get access to your dashboard. Manage your
									work, sign-in, security and much more.{" "}
								</p>
							</div>
							<img
								src={facewithmask}
								className={styles.face_withmask}
							/>
						</div>
					</div>
         ...
         ----------------------------------------------------------------*/}
					...
					{activity === "projects" ? null : (
						<div className='headingLinks'>
							<Link
								...
								className={
									view === "live" ? styles.active : undefined
								}
							>
								Live (
							...
							>
								Paused (
								{statusesCnt?.paused ? statusesCnt?.paused : 0})
							</Link>
							<Link
							...
							>
								Billed (
								{statusesCnt?.billed ? statusesCnt?.billed : 0})
							</Link>
							<Link
							...
							<Link
								...
								className={
									view === "all" ? styles.active : undefined
								}
							>
				...
				{countCheckedProjects ? (
					<div className='selected_project_op'>
						...
						>
							<RiDeleteBin5Line color='blue' size={20} /> &nbsp;
							Delete
						</button>
						<button
							onClick={() => {
						...
						<button>
							<CSVLink
								...
								data={currentSurveys?.filter(survey =>
									checkRows?.includes(
										String(survey?.survey_id)
									)
								)}
							>
								<FiDownload color='blue' size={20} /> &nbsp;
								Export CSV
							</CSVLink>
						</button>
						{/* <button>
				...
				>
					<Box sx={style}>
						<div className={styles.confirmDelete_Modal}>
							...
							{data?.map(sg => (
								<div
									className={styles.selected_dataContainer}
									key={uuid()}
								>
									<sapn className={styles.selected_data}>
										{sg}
									</sapn>
								</div>
							))}
