				...
				{question?.options?.map((option, index) => (
					<div key={uuid()}>
						<FormControlLabel
							control={
								<Checkbox
									checked={displayOpt?.includes(index)}
								/>
							}
							onChange={(e, checked) =>
				...
				<>
					...
					<div className={styles.textfields}>
						<TextField
							...
							onChange={e =>
								handleMinMaxCondition(e.target.value, "min")
							}
						...
						<TextField
							...
							onChange={e =>
								handleMinMaxCondition(e.target.value, "max")
							}
							...
							return (
								<div key={uuid()}>
									<FormControlLabel
										control={
											<Checkbox
												defaultChecked={compulsaryOpt?.includes(
													option
												)}
											/>
										}
				...
				<div className={styles.selected_options}>
					...
					<div className={styles.selected_options_container}>
						{compulsaryOpt?.map(opt => {
							return <span>{question?.options[opt]}</span>
						})}
					</div>
				</div>
			</div>
				...
				{!allowed_responses.length ? (
					...
					allowed_responses.map((response, index) => {
						return (
							<div
								key={response.id}
								className={styles.response_inputs}
							>
								<input
									type='number'
									name='from'
