                                ...
                                <IoMdLock size={20} />
										...
										case "Single Punch":
											return data?.conditions?.valid_options?.map(
												(option, indx) => {
													if (
														data?.conditions?.quotas?.hasOwnProperty(
															indx
														)
													) {
														return (
			...
			>
				<div className={styles.quotas_modal}>
					<p className={styles.title}>
						Change Quota of Qualifications
					</p>
					<input
						type='search'
...
const AddQuotaModal = ({
	...
	const handleInputChange = (e, indx) => {
		...
		if (newData.conditions.quotas === undefined)
			newData.conditions.quotas = {}
		if (isNaN(value)) {
			...
			>
				<div className={styles.quota_add_modal}>
					...
					<p className={styles.question_text}>
						{qualification.question_text}
					</p>
					{(() => {
						if (
							qualification?.question_type ===
							"Numeric - Open-end"
						) {
								...
								(option, indx) => {
									return (
										<div
											key={option}
											className={
												styles.single_punch_condition
											}
										>
											<span>
												{quotaValues?.options?.[option]}
											</span>{" "}
											&nbsp; -
											<input
												type='number'
