...
const ChangeLogComponent = () => {
	...
	useEffect(() => {
		setDisplayChanges(() => {
			return surveyData?.changes?.sort(
				(a, b) =>
					b?.changed_by?.updated_at.toDate() -
					a?.changed_by?.updated_at.toDate()
			)
		})
	}, [surveyData])
	return (
		<>
			<div className='change_log'>
				...
				<p className='change_log_instruction'>
					Review Changes to your survey configurations. See who made
					changes and when
				</p>
