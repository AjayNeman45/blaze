...
function Card({
  ...
  useEffect(() => {
    setCopyText({
      ...
      live_url: `${projectBaseURL}/blaze/${surveydata?.encrypt?.sid}-${surveydata?.encrypt?.pid}-${surveydata?.encrypt?.cid}/lightningStart?SRCID=${supplier_id}&RID=[%rid%]`,
      test_url: `${projectBaseURL}/blaze/${surveydata?.encrypt?.sid}-${surveydata?.encrypt?.pid}-${surveydata?.encrypt?.cid}/lightningStart?SRCID=${supplier_id}&gamma=alpha&RID=$[%rid%]`,
      both_url: `Live url = ${`${projectBaseURL}/blaze/${surveydata?.encrypt?.sid}-${surveydata?.encrypt?.pid}-${surveydata?.encrypt?.cid}/lightningStart?SRCID=${supplier_id}&RID=[%rid%]`}\nTest url = ${`${projectBaseURL}/blaze/${surveydata?.encrypt?.sid}-${surveydata?.encrypt?.pid}-${surveydata?.encrypt?.cid}/lightningStart?SRCID=${supplier_id}&gamma=alpha&RID=[%rid%]`}`,
    });
  }, [surveydata]);
    ...
    <>
          ...
          <DialogContent>
            ...
            >
              <TextField
                ...
                variant="outlined"
                disabled
                value={`${projectBaseURL}/blaze/${surveydata?.encrypt?.sid}-${surveydata?.encrypt?.pid}-${surveydata?.encrypt?.cid}/lightningStart?SRCID=${supplier_id}&RID=[%rid%]`}
            ...
            >
              <TextField
                ...
                variant="outlined"
                disabled
                value={`${projectBaseURL}/blaze/${surveydata?.encrypt?.sid}-${surveydata?.encrypt?.pid}-${surveydata?.encrypt?.cid}/lightningStart?SRCID=${supplier_id}&alpha=gamma&RID=$[%rid%]`}
                // inputProps={{ sx: { color: "#fff" } }}
              />
              <button
                className={styles.copybtn}
      ...
      <div className={styles.card}>
        ...
        <button
          ...
          onClick={() => {
            history.push(
              `/livesurveylogs/${surveyid}?logtype=test&supplier_id=${data?.supplier_account_id}`
            );
          }}
        >
          View Test Log
